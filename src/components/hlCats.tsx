...
const CatSubCat: React.FC<HomeProps> = memo(({ t, i18n, theme, subCat, scrollValueY, index, setLoader, loader }) => {
    ...
    return (
        ...
        >
            <section
                className={`absolute inset-0 bg-cover bg-center transition-transform duration-[450ms] ease-in-out transform md:group-hover:scale-125 ${window.innerWidth < 720 ? ((scrollValueY > (index * 165 + 95) && scrollValueY < (index * 165 + 260)) ? "scale-125" : "scale-100") : ""}`}
                style={{ backgroundImage: `url(/src/img/higherlower/countries/material/${subCat}.jpeg)` }}
            />
            <Link
                id="categoryBox"
                to=""
                className={`active:bg-[#eee] active:text-[#110] ${window.innerWidth < 720 ? ((scrollValueY > (index * 165 + 95) && scrollValueY < (index * 165 + 260)) ? "bg-[#033a] backdrop-blur-[0em] text-[#fffefe]" : "text-[#4b1616] bg-[#ffffffaa] pointer-events-none") : ""} flex flex-col justify-center w-[100%] h-[100%] md:text-[#7e1919] md:hover:text-[#fffefe]  md:bg-[#ffffff8a] md:hover:bg-[#033a] backdrop-blur-[0.2em] hover:backdrop-blur-[0em] transition-all duration-[300ms] md:duration-[450ms] ease-in-out z-[900]`}
