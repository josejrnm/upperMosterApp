...
const Footer: React.FC<LangProps> = ({ t, theme }) => {
    ...
    return (
        <div className={`${tS.bgColorFooter} h-[max-content] w-[auto] pt-4 text-[#8a8b8b] static bottom-0 left-0`}>
            ...
            <div className="text-center w-[100%] flex flex-col self-end justify-center h-[max-content] py-2 mt-4">
                ...
                <section id="social-media" className="w-[100%] h-[min-content] flex flex-col justify-center" >
                    <ul className="flex flex-row justify-center items-center flex-wrap">
                        {icons.map((icon) => (<li key={icon.network} className={`rounded-full w-[min-content] h-[min-content] m-1 border-[#F5E4D7] hover:bg-[#979393] transition-colors duration-[250ms] ease-in hover:ease-out`} title={`${icon.network}`}><SocialIcon key={icon.network} className="socialIcon rounded-full" bgColor="transparent" fgColor={tS.iconColor} network={icon.network} url="" /></li>))}
                    </ul>
                </section>
